...
function resetEntity(entityList, entityID, options) {
  return function (dispatch, getState) {
    if (options && options.preCommit) {
      options.preCommit({
        entityList: entityList, entityID: entityID, dispatch: dispatch, getState: getState
      });
    }
    dispatch({
      type: _constants.types.CDEEBEE_RESET_ENTITY,
      payload: { entityList: entityList, entityID: entityID }
    });
    if (options && options.postCommit) {
      options.postCommit({
        entityList: entityList, entityID: entityID, dispatch: dispatch, getState: getState

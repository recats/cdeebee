...
- **Error kinds** (`CdeebeeErrorKind`): `'http'` (non-ok response, body parsed first), `'network'` (fetch itself threw), `'abort'` (external signal, `cancelation`, a plugin's `false`, or an abort mid-parse of a non-ok body), `'parse'` (body could not be parsed as `responseType`), `'plugin'` (an onRequest/onResponse hook threw), `'normalize'` (normalize threw).
